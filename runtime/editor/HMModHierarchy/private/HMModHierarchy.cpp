...
namespace HARMONY
{
	namespace EDITOR
	{
		...
		bool HMModHierarchy::Initialize()
		{
			...
			IPC->RegisterCallBack(IPC->GetCommandInfo(TSTR("AddGameObject")).id, std::bind(&HMModHierarchy::AddGameObject, this, std::placeholders::_1));
			IPC->RegisterCallBack(IPC->GetCommandInfo(TSTR("CreateSceneObject")).id, std::bind(&HMModHierarchy::CreateSceneObject, this, std::placeholders::_1));
			return true;
		}
		...
		void HMModHierarchy::SendAddedSceneData(const char* name, const char* parent_name)
		{
			...
			IPC->SendData(IPC->GetCommandInfo(TSTR("AddedScene")).id, send_data);
		}
