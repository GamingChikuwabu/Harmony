...
namespace HARMONY
{
	namespace SERIALIZE
	{
		namespace JSON
		{
			...
			void fromjson_recursively(rttr::instance obj2, Value& json_object);
			...
			Document doc;
			IJsonSerializer::IJsonSerializer(const std::string json)
			{
			...
			void IJsonSerializer::operator>>(rttr::instance obj)
			{
                fromjson_recursively(obj, doc);
			}
		}
	}
}